(function () {
  ...
  ui.innerHTML = `
    <style>
      ...
      #sfx-close {
        cursor: pointer; color: #4a5068; font-size: 16px; line-height: 1;
        transition: color .15s;
      }
      #sfx-close:hover { color: #e2e8f0; }
      #sfx-body { padding: 16px; }
      #sfx-status {
        font-size: 12px; color: #8892b0; margin-bottom: 14px; min-height: 16px;
        letter-spacing: .02em;
      }
      #sfx-status span { color: #7c85ff; font-weight: 700; }
      #sfx-bar-wrap {
        background: #1a1d2e; border-radius: 4px; height: 6px; overflow: hidden; margin-bottom: 16px;
      }
      #sfx-bar {
        height: 100%; width: 0%;
      ...
      .sfx-kpi {
        background: #1a1d2e; border-radius: 8px; padding: 10px 12px;
        border: 1px solid #2a2d3a;
      }
      ...
      #sfx-log {
        margin-top: 12px; font-size: 10px; color: #4a5068;
  ...
  function detectColumns() {
    // Strategia 1: leggi aria-label degli header
    const byAria = [];
    ...
    // Strategia 2: leggi il testo visibile delle celle header
    const byText = [];
    ...
    // Strategia 3: fallback dinamico — conta le colonne dalle celle dati
    return null;
  }

  function detectNumColsFromData(collected) {
    // Trova il massimo indice di colonna tra tutte le righe raccolte
    let max = 0;
  ...
  function downloadCSV(collected, headers) {
    // Se headers è null (tutti e 3 i metodi falliti), genera Col0..ColN dai dati
